...
export const AccessManager = () => {
  ...
  // Debug logs
  console.log('🔧 AccessManager renderizado');
  ...
  const filteredDiagrams = diagrams.filter(diagram => 
    ...
    (diagram.shareSettings?.users || []).some(user => user.toLowerCase().includes(searchTerm.toLowerCase()))
  );
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-4">
            ...
            <div className="text-sm text-gray-500">
              {diagrams.length} diagrama(s) • {diagrams.reduce((total, d) => total + (d.shareSettings?.users?.length || 0), 0)} usuário(s)
            </div>
          </div>
        </div>
      </div>
            ...
            <div key={diagram.id} className="bg-white rounded-lg shadow-sm border border-gray-200">
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-gray-500">
                      Criado em {new Date(diagram.createdAt).toLocaleDateString('pt-BR')} • {(diagram.shareSettings?.users || []).length} usuário(s) com acesso
                    </p>
                  </div>
                  <div className="flex items-center space-x-2">
                    <span className={`px-3 py-1 rounded-full text-xs font-medium ${
                      (diagram.shareSettings?.users || []).length > 1 
                        ? 'bg-green-100 text-green-800' 
                        : 'bg-yellow-100 text-yellow-800'
                    }`}>
                      {(diagram.shareSettings?.users || []).length > 1 ? 'Compartilhado' : 'Privado'}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="px-6 py-4">
                <div className="space-y-3">
                  {(diagram.shareSettings?.users || []).map((userEmail, index) => (
                    <div key={index} className="flex items-center justify-between py-2 px-3 bg-gray-50 rounded-lg">
                      ...
                      {(diagram.shareSettings?.users || []).length > 1 && !userEmail.includes('admin') && (
                        <button
        ...
        {filteredDiagrams.length === 0 && (
          <div className="text-center py-12">
            ...
            <p className="text-gray-500">
              Tente ajustar sua busca ou crie um novo diagrama.
            </p>
          </div>
        )}
      </div>
