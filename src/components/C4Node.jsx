...
export const C4NodeComponent = ({ 
  ...
  const handleDoubleClick = useCallback((field) => {
    setIsEditing(field);
  ...
  return (
    <div 
      ...
      `}
      style={{
        width: width || nodeData.width || 180,
        height: height || nodeData.height || 120,
        minWidth: '180px',
        minHeight: '120px'
      }}
      ...
      onClick={(e) => {
        console.log('🎯 NÓ CLICADO!', id, { selected, isPreview });
        // Não usar stopPropagation para permitir que ReactFlow capture o evento
      }}
      onMouseDown={(e) => {
        console.log('🎯 NÓ MOUSE DOWN!', id, { button: e.button, selected });
      }}
    >
      ...
      {/* Connection Handles - Exata do TypeScript que funciona */}
      ...
      {/* Type Selector - só aparece quando selecionado */}
      {selected && (
        <div className="px-3 py-1 border-b bg-gray-50">
          ...
          >
            ...
            <option value="component">⚙️ Component</option>
          </select>
        </div>
      )}
