        ...
        <div className="flex-1 relative w-full h-full">
          ...
          >
            <ReactFlow
              ...
              connectionLineStyle={{ stroke: '#2196f3', strokeWidth: 2, strokeDasharray: '8,4' }}
              isValidConnection={(connection) => {
                console.log('🔍 Validando conexão:', connection);
                
                // Validar se source e target existem
                if (!connection.source || !connection.target) {
                  console.log('❌ Validação falhou: source ou target ausente');
                  return false;
                }
                
                // Não permitir conectar nó a si mesmo
                if (connection.source === connection.target) {
                  console.log('❌ Validação falhou: tentativa de auto-conexão');
                  return false;
                }
                ...
                if (!sourceNode || !targetNode) {
                  console.log('❌ Validação falhou: nó não encontrado');
                  return false;
                }
